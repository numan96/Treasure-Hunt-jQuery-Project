...
$sql = "SELECT pubID, pubname, pubLat, pubLon, pubDesc, pubRating FROM mobile_pubs";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
?>
<br><br>
<input type="text" id="myInput" onKeyUp="searchPubs()" placeholder="Search for Pub.."> 
<br>
<table id="pubTable"><tr class="header"><th style="width:90%;" align="center">Pub Name</th><th></th></tr>
<?php
     // output data of each row
     while($row = $result->fetch_assoc()) {
     
		 echo "<tr><td align='center'>" . $row["pubname"]. "</td>";

?> <td><button type="button" class="button" onClick="window.location.href='PubDetails.php?pubid=<?php echo $row['pubID']; ?>'">View Venue</button></td></tr>
<?php
     }
     echo "</table>";
} else {
     echo "0 pubs available";
}
