...
<?php	 
 ...
 <div class="title"><h2 align="center"><strong>Fenchurch Street</strong></h2></div>
 ...
 <script>
      ...
      function initMap() {
        map = new google.maps.Map(document.getElementById('map'), {
          center: {lat: 51.51046100, lng: -0.07955000},
          zoom: 14
		  // loads map in the selected location
        });
		
		
        //start point fenchurch street
		var RMarker1 = new google.maps.Marker({
			title: 'Apex City of London Hotel, Tap once for more Details. Or Double tap to start the game going to this location. ',
			icon: '5star.png',
          position: {lat:51.51046100 , lng: -0.07955000},
          map: map,
          
        });
		
		var infowindow1 = new google.maps.InfoWindow({
			content: "From the natural light filtered through to the contemporary public spaces and sleek reception area to the high-end amenities in each room, Apex City of London Hotel is dedicated to create memorable stays in the heart of London."
			});
		...
		google.maps.event.addListener(RMarker1, 'dblclick', function() {
			window.open("apexhotelmap.php","_self");
			});
				
			
			
			var RMarker2 = new google.maps.Marker({
			title: 'Crutched Friar Pub, Tap once for more Details. Or Double tap to start the game going to this location. ',
			icon: 'pub2.png',
          position: {lat: 51.51118000, lng: -0.07887200},
          map: map,
          
        });
		
		var infowindow2 = new google.maps.InfoWindow({
			content: "Classic city pub with a vaulted wooden ceiling, designer wallpaper and curry evenings on Tuesdays."
			});
		...
		google.maps.event.addListener(RMarker2, 'dblclick', function() {
			window.open("crutchedfriarbarmap.php","_self");
			});
			
			
		var RMarker3 = new google.maps.Marker({
			title: 'The Folly, Tap once for more Details. Or Double tap to start the game going to this location. ',
			icon: 'fork2.png',
          position: {lat: 51.51118500, lng: -0.08587200},
          map: map,

        });
		
		var infowindow3 = new google.maps.InfoWindow({
			content: "A garden-influenced restaurant, deli and bar, where the seasonal menu has international flavours."
			});
		...
		google.maps.event.addListener(RMarker3, 'dblclick', function() {
			window.open("thefollymap.php","_self");
			});
